...
dependencies above, then replace or add the three files below. Replace `com.example.myapplication`
with your project's package if it differs.

Add your two credentials to `VoiceApplication.kt`, run on a physical device, tap **Start call**,
and allow microphone access. Both credentials come from **Agent Studio › Connector Settings** and are
...
### 1. Initialize once — `VoiceApplication.kt`

Create this file beside `MainActivity.kt`:

```kotlin
// app/src/main/java/com/example/myapplication/VoiceApplication.kt
...
Most integrations need only those two values. `environment` defaults to `Environment.US`, and
`hostIdentifier` defaults to the app's `applicationId`; see [Credentials & configuration](#credentials--configuration)
before overriding either one.

### 2. Register the application — `AndroidManifest.xml`

Keep the manifest generated by Android Studio and add `android:name=".VoiceApplication"` to its
`<application>` element. Here is the complete manifest from a fresh `My Application` project:
...
The app still has to request the dangerous `RECORD_AUDIO` permission at runtime, which the activity
below does.

### 3. Paste the call screen — `MainActivity.kt`

Replace the generated file with this one:
...
The screen holds one `VoiceCall`, observes `call.state`, asks for microphone access before `start()`,
and calls `close()` when it leaves the composition. The visible lifecycle is `Idle → Connecting →
Connected → Ended` or `Failed`; `start()` returning means setup is underway, not that the call is
already connected.
