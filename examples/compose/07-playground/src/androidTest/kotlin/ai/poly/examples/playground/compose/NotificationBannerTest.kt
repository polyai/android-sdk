...
/**
 * End-to-end check for the new-message notification (components/NewMessageNotifier.kt), driven
 * against the live dev backend.
 *
 * Notable behaviors:
 *  - AppRoot mounts the notifier with `NotificationPolicy.WHEN_BACKGROUNDED` — it posts a
 *    notification only when the chat is NOT on screen (lifecycle below RESUMED; see
 *    NewMessageNotifier.kt). So instead of asserting a *foreground* banner, this test connects,
 *    sends a message via the composer, immediately backgrounds the app (home), and polls the
 *    notification shade for the new-message notification raised by the agent's reply.
 ...
 *    the reliable label.
 *  - The connect helper tolerates Start / Resume / Start New by button text. The notifier dedupes
 *    on server messageId, but the reply to a freshly-sent message always has a new id, so stale
 *    state cannot mask it.
 *
 * A resume-does-not-re-notify scenario is intentionally NOT covered here: it would require killing
 * and relaunching the host app, which on Android kills the instrumentation process itself. That
 * scenario is covered at the adb-script level instead.
 */
...
class NotificationBannerTest {
    ...
    /**
     * Exercises the notifier's WHEN_BACKGROUNDED policy (see class KDoc): connect, send, background
     * the app, and the agent's reply must raise a "New message" notification in the shade.
     */
    @Test
    fun newMessageNotification_postedWhileBackgrounded() {
        ...
        // notifies), but this settle ensures it is consumed/marked before we background — keeping
        // the later shade match attributable to the reply. Slept in <=1s steps per the
        // no-long-sleep convention.
    ...
    /** Deadline-poll (250ms steps) for the first object matching ANY of [selectors]. */
    ...
    /**
     * Sends [text] through the composer. The module also sends on a trailing newline
     * (ChatView.InputBar), but the explicit send button (content description "Send message") is
     * the steadier black-box path.
     */
    private fun send(text: String) {
        val composer: UiObject2 = checkNotNull(device.findObject(testTagSelector("composer"))) {
            "composer present"
        }
        composer.click()
        composer.setText(text) // works on Compose text fields via the SetText semantics action
        checkNotNull(device.findObject(By.desc("Send message"))) { "send button present" }.click()
    }

    // MARK: - Evidence

    /** Saves a screenshot under the instrumentation-args dir. */
