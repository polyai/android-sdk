...
/**
 * Three leading, pulsing rounded rows that preview where the first agent messages will land — shown
 * while the session is opening and no messages exist yet (the caller gates this on
 * `!isReady && messages.isEmpty()`). Decorative — hidden from accessibility. Self-manages its pulse:
 * animates while attached + visible, stops otherwise.
 *
 * The rows are BOTTOM-aligned: this list uses `stackFromEnd` (messages bottom-pinned), so the
 * placeholder sits where the first messages will actually land.
 */
class LoadingSkeletonView @JvmOverloads constructor(
    ...
    init {
        ...
        gravity = Gravity.BOTTOM // match the stackFromEnd chat list (see class doc)
