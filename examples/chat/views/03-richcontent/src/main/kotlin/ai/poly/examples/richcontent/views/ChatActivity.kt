...
import ai.poly.examples.richcontent.views.databinding.ActivityChatBinding
...
/**
 * Rung 03 (Views) — rich content. Everything from 02-Standard plus image attachments, URL link
 * cards, `tel:` call actions, Markdown rendering (`ChatAdapter`/`RichTextSpans`), retryable image
 * loading (`RetryableImageView`), and a new-message notifier (`NewMessageNotifier`) that by default
 * banners only while the chat is backgrounded.
 */
class ChatActivity : ComponentActivity() {
    ...
    // New-message notifier (see NewMessageNotifier.kt) + its runtime permission.
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // reading it). Switch to NotificationPolicy.ALWAYS or .NEVER. See NewMessageNotifier.kt.
        val notificationPolicy = NotificationPolicy.WHEN_BACKGROUNDED
        if (notificationPolicy != NotificationPolicy.NEVER &&
    ...
    private fun refresh() {
        // Decide whether to follow BEFORE the list changes: follow only if the user is already near
        // the bottom, or just sent. Otherwise surface the pill.
