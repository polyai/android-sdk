...
/**
 * Renders the agent Markdown path: HTML normalization + Markdown parsing.
 * Agent text arrives raw — usually Markdown (`**bold**`, `*italic*`, `` `code` ``, `[links](url)`),
 * but it can carry a small HTML subset (most often `<br>`) because the backend serves the same content
 * to the web chat widget. We normalize that HTML allow-list to newlines + Markdown, then fold
 * `[text](url)` links + bold/italic/code into a [Spanned]. Bare `https://…` URLs
 * are NOT auto-linkified (only explicit Markdown links). Set the result on a TextView with
 * `LinkMovementMethod` so links are tappable.
 */
...
/** Folds `**bold**` / `*italic*` / `` `code` `` over a plain span (in parser priority order). */
