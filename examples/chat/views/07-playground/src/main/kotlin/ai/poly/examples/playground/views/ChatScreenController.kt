...
/**
 * The chat surface plus the playground extras: an optional always-on debug strip at the top
 * and optional iMessage-style timestamp separators interleaved into the message list (the
 * list rows gain a Timestamp kind so separators can be inserted between message groups).
 */
class ChatScreenController(
    ...
    private val showDebugStrip: Boolean = false,
    private val showTimestamps: Boolean = false,
    private val diagnostics: DevDiagnostics? = null,
) {
    companion object {
    ...
    // A clean terminal close (debug 1000 sim, or the server closing 1000 without a SESSION_END)
    // latches the SDK's send gate shut and never reconnects — treat it as ended in the UI too.
    ...
    private var debugStrip: DebugStripView? = null
    ...
    fun start() {
        ...
        // Debug strip pinned above the banner stack.
        if (showDebugStrip && diagnostics != null) {
            val strip = DebugStripView(activity)
            debugStrip = strip
            ...
            strip.start(activity.lifecycleScope, diagnostics)
        }
        ...
        bindJob = activity.lifecycleScope.launch {
            activity.repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                launch {
                    ...
                    // optimistic. Only the ended (or terminally closed) state swaps the input bar.
