...
/**
 * Three leading, pulsing rounded rows that preview where the first agent messages will land —
 * shown while the session is opening and no messages exist yet (the caller gates this on
 * `!isReady && messages.isEmpty()`). Decorative — hidden from accessibility. Self-manages its
 * pulse: animates while attached + visible, stops otherwise.
 *
