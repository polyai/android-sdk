...
/**
 * An ImageView that loads from a URL via Coil with tap-to-retry on failure and a 5-second
 * auto-retry that re-arms every cycle (a persistently-failing image keeps retrying). The SDK
 * never fetches bytes — you load the URL. (Coil owns memory/disk caching.)
 *
...
class RetryableImageView @JvmOverloads constructor(
    ...
    private fun reload() {
        ...
        disposable?.dispose()
        // Clear the image and spin while the request is in flight — without this, a retry
        // stretches the small failure glyph across the card for the request's duration.
        if (circle) {
            scaleType = ScaleType.CENTER_CROP
            // keep the avatar placeholder while (re)loading
        } else {
            scaleType = ScaleType.CENTER
            setImageDrawable(
                androidx.swiperefreshlayout.widget.CircularProgressDrawable(context).apply {
                    setColorSchemeColors(Palette.systemGray2)
                    start()
                },
            )
        }
