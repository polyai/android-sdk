...
import ai.poly.examples.resilience.views.databinding.ActivityChatBinding
...
/**
 * Rung 04 (Views) — resilience. The classic-Views resilience screen:
 * everything from 03-RichContent plus a device-offline banner stacked above the
 * SDK's reconnect banner (`NetworkMonitor`), a pre-handshake loading skeleton (`LoadingSkeletonView`),
 * and a full-screen terminal-error screen (with manual retry) when the SDK gives up reconnecting.
 * (04 drops 03's new-message notifier.)
 */
class ChatActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Keep the composer above the gesture nav bar (closed) / keyboard (open).
    ...
    private fun bind() {
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                launch {
                    // Skeleton gate: !isReady && messages.isEmpty (warm resumes have messages → skip).
                    session.isReady.collect { ready ->
                        latestReady = ready
                        updateSkeleton()
                    }
                }
                ...
                launch {
                    // Terminal failure → full-screen retry screen; the End button steps aside for it.
    ...
    private fun buildItems(): List<ListItem> {
        ...
        msgs.forEach { m ->
            val pending = m is ChatMessage.User && m.message.delivery == Delivery.PENDING
            items.add(ListItem.Message(m, showSendingLabel = pending))
        }
    ...
    private fun updateSkeleton() {
        ...
        // arrived. On warm resume, messages are already in memory so we skip the skeleton entirely.
    ...
    override fun onPrepareOptionsMenu(menu: Menu): Boolean {
        // Hidden once the chat ends or the terminal-error screen takes over.
        menu.findItem(MENU_END)?.isVisible = !endedState && !failureActive
