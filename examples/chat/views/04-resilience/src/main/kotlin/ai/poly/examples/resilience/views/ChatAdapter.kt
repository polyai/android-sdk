...
import ai.poly.examples.resilience.views.databinding.ItemMessageBinding
import ai.poly.examples.resilience.views.databinding.ItemSuggestionsBinding
import ai.poly.examples.resilience.views.databinding.ItemTypingBinding
...
/**
 * RecyclerView adapter backed by a diffable data source: message bubbles, a
 * suggestions row under the last agent message, and a typing-indicator footer row.
 */
class ChatAdapter(
    ...
    class MessageHolder(private val b: ItemMessageBinding) : RecyclerView.ViewHolder(b.root) {
        ...
        // Each carousel is pinned to 0.85 of the content width so cards show side-by-side and scroll.
        ...
        // Rung 04 system-event text — note these strings differ from the other rungs.
        private fun systemText(event: SystemEvent): String = when (event) {
            ...
            is SystemEvent.QueueStatus -> event.displayMessage ?: (event.position?.let { "Position #$it in queue" } ?: "Queued…")
